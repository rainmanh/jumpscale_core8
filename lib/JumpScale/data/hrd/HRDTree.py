...
class HRDTree(HRDBase):
    ...
    def add2treeFromContent(self, content):
        hrd = HRD("", treeposition, self,
                  prefixWithName=self.prefixWithName, keepformat=self.keepformat)
    ...
    def add2tree(self, path, recursive=True):
        ...
        for pathfound in paths:
            ...
            hrd = HRD(pathfound, self, prefixWithName=self.prefixWithName,
                      keepformat=self.keepformat)
            self.hrds.append(hrd)

    def getHrd(self, key):
        if key not in self.items:
            raise j.exceptions.Input(
                "Cannot find key:'%s' in tree" % key, "hrdtree.gethrd.notfound")
