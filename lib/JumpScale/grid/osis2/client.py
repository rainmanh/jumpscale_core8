...
def Struct(*args, **kwargs):
    ...
    return type(name, (ModelObject,), {'__init__': init, '__slots__': list(kwargs.keys())})
...
class Client(object):
    ...
    def _export(self, namespace, categoryname, outputpath, session=None):
        # https://github.com/Jumpscale/jumpscale_core8/blob/master/lib/JumpScale/grid/osis/OSISCMDS.py
        raise NotImplemented()
...
class ObjectClient(object):
    ...
    def list(self):
        ...
        url = "%s?%s" % (self._baseurl, urllib.parse.urlencode(query))
