...
class CockpitDeployer:
    ...
    def _register_domain(self, vdc_adress):
        ...
        def validateDNS(dns_name):
            self.printInfo("Test if dns name is available (%s)" % dns_name)
            exists, host = dns_client.exists(dns_name)
            if exists and host != vdc_adress:
                self.printInfo("%s is not available, please choose another name")
                return False
            else:
                return True
    ...
    def deploy(self):
        ...
        self.printInfo('created at %s' % cockpit_repo_path)

        self.printInfo("Create cockpit VM")
        ...
        self.printInfo("Configuration of shellinabox")
        ...
        self.printInfo("Configuration of caddy proxy")
