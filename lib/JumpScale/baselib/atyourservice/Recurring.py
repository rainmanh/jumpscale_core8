...
class RecurringItem():
    def __init__(self, service,name,period,last):
        ...
        self.name = name
        self.period = period
        self.periodSec = j.base.time.getDeltaTime(self.period)
        self.last = last

    def __repr__(self):
        if self.last != 0:
            return str("|%-20s | %-10s | %-10s | %-30s|\n" % (self.name,self.period,self.last,j.base.time.epoch2HRDateTime(self.last)))
        else:
...
class Recurring():
    def __init__(self, service):
        ...
        self.path = j.system.fs.joinPaths(self.service.path, "recurring.md")
        self.items={}
        self._read()
    ...
    def _read(self):
        pastHeader = False
        if j.system.fs.exists(path=self.path):
            for item in j.system.fs.fileGetContents(self.path).split("\n"):
                if not pastHeader:
                    if item.find('---') != -1:
                        pastHeader = True
                    continue
    ...
    def write(self):
        if self.items != {}:
            j.system.fs.writeFile(filename=self.path, contents=str(self))
