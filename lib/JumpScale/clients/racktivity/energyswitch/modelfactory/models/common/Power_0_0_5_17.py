...
from .Power_0_0_5_4 import Model as Power
import struct, time
...
class Model(Power):
    ...
    def getOscilloscopeFrequencyData(self, moduleID, portnumber=1):
        ...
        for i in range(0, 256, 4):
            result['voltage']['amplitudes'].append(struct.unpack('<H', voltageSamples[i:i+2])[0] * voltageCalibration)
            result['voltage']['phases'].append(struct.unpack('<h', voltageSamples[i+2:i+4])[0])
            result['current']['amplitudes'].append(struct.unpack('<H', currentSamples[i:i+2])[0] * currentCalibration)
