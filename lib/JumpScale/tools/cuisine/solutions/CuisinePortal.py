...
class CuisinePortal(base):
    ...
    def install(self, start=True, reset=False):
        ...
        self._cuisine.core.file_copy('%s/github/jumpscale/jscockpit/jscockpit/api_server' %
                                     j.dirs.codeDir, '%s/ays_api/api_server' % j.dirs.appDir)
        self._cuisine.core.file_copy('%s/github/jumpscale/jscockpit/jscockpit/ays_api' %
                                     j.dirs.codeDir, '%s/ays_api/ays_api' % j.dirs.appDir, recursive=True)
        if start:
            self.start()
