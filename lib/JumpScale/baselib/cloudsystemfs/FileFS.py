...
class FileFS:
    ...
    def __init__(self, end_type, path, is_dir=False, recursive=False, tempdir=j.dirs.TMPDIR, Atype='copy'):
        ...
        self.logger.debug("FileFS: path [%s] file [%s]" % (
            self.path, self.local_file))
        self.is_dir = is_dir
    ...
    def upload(self, uploadPath):
        ...
        if self.Atype == "move":
            if self.is_dir:
                if self.recursive:
                    self.logger.info("FileFS: (directory) Copying [%s] to path [%s] (recursively)" % (
                        uploadPath, self.path))
                    j.sal.fs.moveDir(uploadPath, self.path)
                else:
                    # walk tree and move
                    for file in j.sal.fs.walk(uploadPath, recurse=0):
                        self.logger.info(
                            "FileFS: (directory) Copying file [%s] to path [%s]" % (file, self.path))
                    ...
                    if self.recursive:
                        ...
                        if j.sal.fs.isDir(uploadPath):
                            j.sal.fs.copyDirTree(
                                uploadPath, self.path, update=True)  # was copyDir !!
                        else:
                            # was copyDir !!
