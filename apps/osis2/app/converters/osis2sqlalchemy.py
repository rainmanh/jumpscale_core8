...
def generateDomainFromSpecFile(namespace, spec):
    ...
    sqlalchemyfolder = j.sal.fs.joinPaths(tmplDir, '..', '..', 'models', 'sqlalchemy')
    sqlalchemyfolder = os.path.abspath(sqlalchemyfolder)
    namespacefile = j.sal.fs.joinPaths(sqlalchemyfolder, '%s.py' % namespace)
    j.sal.fs.writeFile(namespacefile, result)
