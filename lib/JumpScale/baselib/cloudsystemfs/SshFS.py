...
class SshFS:
    ...
    def _connect(self):
        ...
        self.logger.info("SshFS: mounting share [%s] from server [%s] with credentials login [%s] and password [%s]" % (
            self.directory, self.server, self.username, self.password))

        command = "echo \"%s\" | %s %s@%s:%s %s  -o password_stdin -o StrictHostKeyChecking=no" % (
            self.password, self._command, self.username, self.server, self.directory, self.mntpoint)
            ...
            if self.is_dir:
                if self.recursive:
                    ...
                    for file in j.sal.fs.walk(uploadPath, recurse=0):
                        self.logger.info(
                            "SshFS: uploading directory -  Copying file [%s] to path [%s]" % (file, self.mntpoint))
                        j.sal.fs.moveFile(file, self.mntpoint)
            else:
                self.logger.info(
                    "SshFS: uploading file - [%s] to [%s]" % (uploadPath, self.mntpoint))
                j.sal.fs.moveFile(uploadPath, j.sal.fs.joinPaths(
                    self.mntpoint, self.filename))
        else:
            if self.Atype == "copy":
                if self.is_dir:
                    if self.recursive:
                        j.sal.fs.copyDirTree(
                            uploadPath, self.mntpoint, update=True)
                    else:
                        # walk tree and copy
                        for file in j.sal.fs.walk(uploadPath, recurse=0):
                            self.logger.info(
                    ...
                    self.logger.info(
                        "SshFS: uploading file - [%s] to [%s]" % (uploadPath, self.mntpoint))
                    j.sal.fs.copyFile(uploadPath, j.sal.fs.joinPaths(
                        self.mntpoint, self.filename))
    ...
    def list(self):
        ...
        if self.path_components:
            if len(self.path_components) > 1:
                ...
                if os.path.isdir(self.path_components[-1]):
                    ...
                    raise j.exceptions.RuntimeError('%s is not a valid directory under %s' % (
                        '/'.join(self.path_components), self.sharename))
        ...
        self.logger.info("list: Returning content of SSH Mount [%s] which is tmp mounted under [%s]" % (
            self.share, self.mntpoint))
