...
class TelegramAYS():
	...
	def executeProgressive(self, bot, update, command):
		...
		process = j.sal.process.execute(command, outputToStdout=False, useShell=False, dieOnNonZeroExitCode=False, async=True)
	...
	def _initRepo(self, username, project):
		...
		j.sal.process.execute("git init", outputToStdout=False, dieOnNonZeroExitCode=False)
