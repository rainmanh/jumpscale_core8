...
class Xonsh:
    ...
    def config(self):
        ...
        """
        j.do.writeFile(j.sal.fs.joinPaths(j.dirs.homeDir,".xonshrc"),C)




    def configTmux(self,restartTmux=False):
        C="""
        ...
        j.do.writeFile(j.sal.fs.joinPaths(j.dirs.homeDir,".tmux.conf"),C)

        if restartTmux:
            j.do.execute("killall tmux",dieOnNonZeroExitCode=False,outputStdout=False, outputStderr=False)
