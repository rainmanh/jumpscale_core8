...
class AgentControllerFactory(object):
    ...
    def getInstanceConfig(self, instance=None):
        ...
        service = None
        services = j.atyourservice.findServices(role='agentcontroller_client', instance=instance)
        if len(services) <= 0:
            service = j.atyourservice.findServices(role='agentcontroller_client', first=True)
        else:
            service = services[0]
