...
from JumpScale import j
from JumpScale.core.types.base import BaseType as TypeBaseType
...
def generate_init_properties(cls, attrs):
    ...
    def __init_properties__(self):
        ...
        for name, attr in (p for p in list(attrs.items()) \
...
class BaseTypeMeta(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for name, value in (p for p in list(attrs.items()) \
                            if isinstance(p[1], TypeBaseType)):
            value._PM_NAME = name
