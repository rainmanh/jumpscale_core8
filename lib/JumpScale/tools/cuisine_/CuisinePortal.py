...
class CuisinePortal(object):
    ...
    def linkCode(self, minimal=False):
        ...
        destjslib = self.cuisine.core.run("js 'print(j.do.getPythonLibSystem(jumpscale=False))'", showout=False)

        self.cuisine.core.file_link("%s/github/jumpscale/jumpscale_portal8/lib/portal" % self.cuisine.core.dir_paths["codeDir"], "%s/portal" % destjslib, symbolic=True, mode=None, owner=None, group=None)
        ...
        self.cuisine.core.file_link("%s/github/jumpscale/jumpscale_portal8/jslib" % self.cuisine.core.dir_paths["codeDir"], '%s/jslib' % self.portal_dir)
        ...
        self.cuisine.core.dir_ensure(self.example_portal_dir)

        if not minimal:
            for space in self.cuisine.core.fs_find("%s/github/jumpscale/jumpscale_portal8/apps/gridportal/base" % self.cuisine.core.dir_paths["codeDir"],recursive=False):
                spacename = j.sal.fs.getBaseName(space)
                if not spacename == 'home':
                    self.cuisine.core.dir_ensure(j.sal.fs.joinPaths(self.example_portal_dir, 'base', 'gridportal'))
                    self.cuisine.core.file_link(space, j.sal.fs.joinPaths(self.example_portal_dir, 'base', 'gridportal', spacename))

        self.cuisine.core.dir_ensure('%s/base/home/.space' %self.example_portal_dir)
        self.cuisine.core.file_ensure('%s/base/home/home.md' %self.example_portal_dir)
        ...
        self.cuisine.core.file_copy(j.sal.fs.joinPaths(self.cuisine.core.dir_paths["codeDir"], 'github/jumpscale/jumpscale_portal8/apps/portalbase/portal_start.py'), self.example_portal_dir)
        ...
        self.cuisine.core.file_write(j.sal.fs.joinPaths(self.example_portal_dir,'config.hrd'), content=str(configHRD))
