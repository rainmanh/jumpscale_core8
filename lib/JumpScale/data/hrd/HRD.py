                ...
                for item in items:
                    ...
                    if self.hrd.exists(item2):
                        replacewith = j.data.text.pythonObjToStr(
                            self.hrd.get(item2), multiline=False, partial=partial)
                        ...
                        replacewith = j.data.text.pythonObjToStr(self.hrd.get(
                            "%s.%s" % (self.hrd.name, item2)), multiline=False, partial=partial)
                        data = data.replace(item, replacewith)
                        # data=data.replace("//","/")
        ...
        if self.ttype == "str" or self.ttype == "base":
                ...
                for item in self.value.split(","):
                    ...
                    if item.find(":") == -1:
                        raise j.exceptions.Input(
                            "In %s/%s: cannot parse:'%s', need to find : to parse dict" % (self.hrd.name, self.name, item))
                    key, post2 = item.split(":", 1)
                    currentobj[key.strip()] = j.data.text.machinetext2val(
                        post2.strip())
            ...
            if self.value.strip() == "":
                ...
                for item in self.value.split(","):
                    if item.strip() == "":
                        continue
                    currentobj.append(j.data.text.machinetext2val(
...
class HRD(HRDBase):
    ...
    def __init__(self, path=None, tree=None, content="", prefixWithName=False, keepformat=True, args={}, templates=[], istemplate=False):
        ...
        if self.path is not None:
            self.name = ".".join(
                j.sal.fs.getBaseName(self.path).split(".")[:-1])
        else:
            self.name = ""
        self.tree = tree
        self.changed = False
        ...
        if self.prefixWithName:
            if self.name == "":
                raise j.exceptions.RuntimeError(
                    "name cannot be empty when prefixWithName used.")
            key = key.replace('%s.' % self.name, '')
        if key not in self.items:
            self.items[key] = HRDItem(
                name=key, hrd=self, ttype=ttype, data=value, comments="")

        self.items[key].set(value, persistent=persistent,
                            comments=comments, temp=temp, data=data)

    def get(self, key, default=None, ask=True):
        if key not in self.items:
            if default == None:
                msg = "Cannot find value with key %s in tree %s." % (
            ...
            if key2 in self.items:
                if templateItem.data.find("@ASK") == -1:
                    ...
                    if self.items[key2].data != templateItem.data and templateItem.data.strip() != "":
                        print("we overrule: %s with '%s'" %
                              (key2, templateItem.data))
                ...
                # its not in hrd yet
                if templateItem.data.find("@ASK") == -1:
                    self.set(key2, value=templateItem.value, persistent=False,
                             comments=templateItem.comments, ttype=templateItem.ttype, data=templateItem.data)
                else:
                    self.set(key2, value="", persistent=False, comments=templateItem.comments,
                             ttype=templateItem.ttype, data=templateItem.data)
        ...
        while True:
            ...
            if state == "multiline":
                ...
                if vartype.startswith("binary"):
                    ...
                    # if post was empty then we need to process current line
                    # again
                    if post.strip() == "":
                        x = x - 1
                    # end of multiline var needs to be processed
            ...
            if state == "multiline":
                ...
                if vartype == "unknown":
                    raise j.exceptions.RuntimeError(
                        "parse error, only dict, list, normal or ask format in multiline")
