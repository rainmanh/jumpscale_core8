...
== == == =
        ...
        if self.core.isMac:
            ...
            self.cuisine.core.run(
                'cd {builddir}; unzip  -u {builddir}/caddy_{os}_amd64.zip'.format(builddir=self.builddir, os=os))
        else:
            ...
            self.cuisine.core.file_download(caddy_url, dest, minsizekb=4)
            self.cuisine.core.run(
                'cd {builddir}; tar xvf {builddir}/caddy_{os}_amd64.tar.gz'.format(builddir=self.builddir, os=os))
        self.cuisine.core.file_copy(
            '{builddir}/caddy_{os}_amd64'.format(builddir=self.builddir, os=os), '$BINDIR/caddy')
        self.cuisine.bash.profileDefault.addPath(self.cuisine.core.dir_paths['BINDIR'])
        self.cuisine.bash.profileDefault.save()
        addr = ':8000'
