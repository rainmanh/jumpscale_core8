...
class NetworkScanner(object):
    ...
    def __init__(self, cidr, port=80):
        code, _ = j.sal.process.execute('which nmap', outputToStdout=False, dieOnNonZeroExitCode=False)
        if code != 0:
    ...
    def scan(self):
        ...
        code, output = j.sal.process.execute(cmd, outputToStdout=False, dieOnNonZeroExitCode=False)
        if code != 0:
