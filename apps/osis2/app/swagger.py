...
def expose_docs(app, static_path):
    ...
    def specs():
        ...
        catalog['info'] = {'description': 'Autogenerated swagger for ROS', 'version': '7.0', 'title': 'JumpScale OSIS2 API'}
        ...
        for modelname, model in cfg['domains'].items():
            catalog['tags'].append({'name': modelname, 'description': modelname})
            for path, pathmethods in model.items():
                newpathmethods = {}
                for k, v in pathmethods.items():
                    if 'params' in v:
                        params = v.pop('params')
                        for param in params:
                            type = param.get('type')
                ...
                for methodname, methodinfo in newpathmethods.items():
                    res = {'type': 'string'}
