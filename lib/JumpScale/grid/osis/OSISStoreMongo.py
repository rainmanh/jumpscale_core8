...
class OSISStoreMongo(OSISStore):
    ...
    def __init__(self, dbconnections):
        connection = None
        for k, v in dbconnections.items():
            if k.find('mongodb') != -1:
                connection = v
                break
        if connection is None:
            j.events.opserror_critical(msg="can't find any valid conection to mongodb.", category="OSISStoreMongo.init")

        self.dbclient = connection
