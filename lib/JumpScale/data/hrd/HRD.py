...
class HRDItem:
    ...
    def get(self, ask=True):
        ...
        if self.value is None or (j.data.types.string.check(self.value) and self.value.strip("'").strip() == ""):
            ...
            if len(items) > 0:
                for item in items:
                    ...
                    if self.hrd.exists(item2):
                        ...
                        # data=data.replace("//","/")
                    elif self.hrd.prefixWithName and self.hrd.tree is not None and self.hrd.tree.exists("%s.%s" % (self.hrd.name, item2)):
...
class HRD(HRDBase):
    ...
    def __init__(self, path=None, tree=None, content="", prefixWithName=False,
                 keepformat=True, args={}, templates=[], istemplate=False):
        self.path = path
