...
class FtpFS:
    ...
    def __init__(self, end_type, server, path, username, password, is_dir=False, recursive=False, tempdir=j.dirs.tmpDir, Atype='copy'):
        ...
        self.logger.info("FtpFS: connection information: server [%s] path [%s] username [%s] password [%s]" % (
        ...
        self.local_dir = j.sal.fs.joinPaths(
            tempdir, j.data.idgenerator.generateGUID())
        ...
        if is_dir == False:
            ...
            self.logger.info("FtpFS: copying to local directory [%s] from path [%s]" % (
    ...
    def _connect(self, dontCD=False):
        try:
            ...
            raise j.exceptions.RuntimeError('Failed to login on ftp server [%s] credentials login: [%s] pass [%s]' % (
    ...
    def exists(self):
        ...
        if not self.is_dir:
            try:
                self.ftp.sendcmd("MDTM %(fileName)s" %
    ...
    def retrieveFile(self, file, dir, dest):
        ...
        self.logger.info(
            "FtpFS: retrieving [%s] from dir [%s] to [%s]" % (file, dir, lfile))
    ...
    def storeFile(self, file, uploadPath):
        ...
        # print "%s:%s:%s %s
        # %s"%(self.server,self.username,self.password,file,uploadPath)
        ...
        if size != stat.st_size:
            ...
            raise j.exceptions.RuntimeError(
                "Could not upload:%s %s, size different, have now deleted" % (file, uploadPath))
    ...
    def handleUploadDir(self, dir, upload_path):
        ...
        self.logger.info("FtpFS: dirname is %s and upload path %s" %
                         (dname, upload_path))
        fname = dir.replace(upload_path, '')
        previous_dir = '/'.join(['/', self.path])
        # creating directory on FTP
        self.logger.info(
            "FtpFS: mkd and cwd to [%s] (previous dir %s)" % (fname, previous_dir))
        self.ftp.mkd(fname)
        # self.ftp.cwd(fname)
...
#       d = j.sal.fs.listDirsInDir(uploadPath,self.recursive)
    ...
    def handleDownloadDir(self, dirname):
        ...
        self.logger.info(
            "FtpFs: handleDownloadDir - creating local [%s]" % rdir)
        j.sal.fs.createDir(rdir)
        ...
        for l in listing:
            ...
            if len(name) > 0:
                if t[0].startswith('d'):  # WARNING: dirty-hack alarm!
                    ...
                    self.logger.info(
                        "FtpFS: normal file, attempting to retrieve file [%s] to location [%s]" % (name[0], rdir))
                    self.retrieveFile(name[0], self.path, rdir)
            else:
                self.logger.info("FtpFS: skipping [%s]" % name)
    ...
    def list(self):
        ...
        dir_content = []
        self.logger.info(
            "list: Returning list of FTP directory [%s]" % self.path)
        listing = []
