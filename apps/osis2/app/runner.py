...
def hookevents(namespace, app):
    ...
    try:
        ...
        for _, hookmethod in HOOKSMAP.items():
    ...
    for model in list(app.settings['DOMAIN'].keys()):
        ...
        for dbevent, hookmethod in HOOKSMAP.items():
...
def start(basedir, hrd):
    ...
    namespaces = []
    for type_, specs in fullspecs.items():
        for namespace, models in specs.items():
            if type_ == 'sql':
    ...
    for namespace, models in spaces_models.items():
