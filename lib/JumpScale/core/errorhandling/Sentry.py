def Sentry():
    ...
    def sendMessageToSentry(self, modulename, message, ttype="bug", tags=None, extra={}, level="error", tb=None, frames=[], backtrace="", hrdprefix="sentry"):
        ...
        if j.application.config.exists("%s.server" % hrdprefix):
            ...
            if modulename == None:
                ...
                try:
                    ...
                    while ignore(modulename):
                        ...
                        modulename = modulename.replace("<module ", "").replace(
                            "'", "").replace(".pyc", "").replace(">", "")
                        try:
                    ...
                    if modulename.find("appname") == -1:
                        modulename = "appname:%s / %s" % (
                            j.application.appname, modulename)
            ...
            try:
                r = requests.post(url2, data=j.data.serializer.json.dumps(
                    data), headers=headers, timeout=1)
            except Exception as e:
                pass
