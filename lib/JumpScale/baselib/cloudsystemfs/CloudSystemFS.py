...
class CloudSystemFS:
    ...
    def copyFile(self, sourcepath, destinationpath, tempdir=j.dirs.tmpDir):
        ...
        self.logger.info("copyFile: from [%s] to [%s]" % (
    ...
    def moveFile(self, sourcepath, destinationpath, tempdir=j.dirs.tmpDir):
        ...
        Move a file
        """
        self.logger.info("moveFile: from [%s] to [%s]" % (
            sourcepath, destinationpath))
    ...
    def _getSourceHandler(self, sourcepath, is_dir=False, recursive=True, tempdir=j.dirs.tmpDir, Atype='copy'):
        ...
        # Determine the object we need to call
        self.logger.info(
            "_getSourceHandler: source protocol [%s]" % (src_proto))

        # for the source
        if(src_proto == "cifs" or src_proto == "smb"):
            src_fs = CifsFS('src', server=src_elements.hostname, share=src_elements.path, username=src_elements.username,
                            password=src_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (src_proto == "ftp"):
            src_fs = FtpFS('src', server=src_elements.hostname, path=src_elements.path, username=src_elements.username,
                           password=src_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (src_proto == 'file'):
            src_fs = FileFS('src', path=src_elements.path, is_dir=is_dir,
                            recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (src_proto == 'http'):
            src_fs = HttpFS('src', server=src_elements.netloc,
                            path=src_elements.path, tempdir=tempdir, Atype=Atype)
        elif (src_proto == 'sftp'):
            src_fs = SshFS('src', server=src_elements.hostname, directory=src_elements.path, username=src_elements.username,
                           password=src_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        else:
            q.eventhandler.raiseError(
                'Unsupported protocol [%s] for the sourcepath [%s]' % (src_proto, sourcepath))
    ...
    def _getDestinationHandler(self, destinationpath, is_dir=False, recursive=True,
        ...
        # Determine the object we need to call
        self.logger.info(
            "_getDestinationHandler: destination protocol [%s]" % (dst_proto))

        if(dst_proto == "cifs" or dst_proto == "smb"):
            dst_fs = CifsFS('dst', server=dst_elements.hostname, share=dst_elements.path, username=dst_elements.username,
                            password=dst_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (dst_proto == 'ftp'):
            dst_fs = FtpFS('dst', server=dst_elements.hostname, path=dst_elements.path, username=dst_elements.username,
                           password=dst_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (dst_proto == 'file'):
            dst_fs = FileFS('dst', path=dst_elements.path, is_dir=is_dir,
                            recursive=recursive, tempdir=tempdir, Atype=Atype)
        elif (dst_proto == 'http'):
            raise j.exceptions.RuntimeError(
                'http as a destination is not supported')
        elif (dst_proto == 'sftp'):
            dst_fs = SshFS('dst', server=dst_elements.hostname, directory=dst_elements.path, username=dst_elements.username,
                           password=dst_elements.password, is_dir=is_dir, recursive=recursive, tempdir=tempdir, Atype=Atype)
        else:
            q.eventhandler.raiseError(
    ...
    def _parseCifsURL(self, url):
        ...
        """
        # Warning: Dirty HACK since urllib does not support cifs/smb but the
        # URI is the same as ftp or http
        durl = url
    ...
    def exportDir(self, sourcepath, destinationpath, recursive=True, tempdir=j.dirs.tmpDir):
        ...
        # Tested:
        # j.sal.cloudfs.importDir('smb://autotest:phun5chU@fileserver.aserver.com/Public/Engineering/vdi/sso_images/SSO_VD',
        # 'file:///tmp/tmp/')
    ...
    def exportVolume(self, sourcepath, destinationpath, format="vdi", tempdir=j.dirs.tmpDir):
        ...
        if not tempdir and self._determineProtocol(destinationpath) in ("cifs", "smb"):
            destination = self._getDestinationHandler(
        ...
        self.logger.info("CloudSystemFS: exportVolume source [%s] to path [%s]" % (
            sourcepath, tmp_outputFileName))
        ...
        j.sal.qemu_img.convert(
            sourcepath, 'raw', tmp_outputFileName, format, compressTargetImage=compressImage)
    ...
    def importVolume(self, sourcepath, destinationpath, format='vdi', tempdir=j.dirs.tmpDir):
        ...
        """
        prefix = 'file://'
        self.logger.info("CloudSystemFS: importVolume source [%s] to path [%s]" % (
            sourcepath, destinationpath))
        ...
        if protocol == "file":
            ...
            self.logger.info(
                "Source is a local file:// not running copyFile... for %s" % elements.path)
            tmp_inputFileName = elements.path
        elif protocol == "smb" or protocol == "cifs":
            src_elements = self._parseCifsURL(sourcepath)
            src_fs = CifsFS('src', server=src_elements.hostname, share=src_elements.path, username=src_elements.username,
                            password=src_elements.password, is_dir=False, recursive=False, tempdir=tempdir)
            tmp_inputFileName = src_fs.download()
            self.logger.info(
                "Source is a CIFS/SMB share, not running copyFile,using %s" % tmp_inputFileName)
        else:
            tmp_inputFileName = j.sal.fs.getTempFileName(tempdir)
            self.copyFile(sourcepath, ''.join(
                [prefix, tmp_inputFileName]), tempdir=tempdir)

        j.sal.qemu_img.convert(
            tmp_inputFileName, format, destinationpath, 'raw', compressTargetImage=compressImage)
    ...
    def listDir(self, path):
        ...
        if(proto == "cifs" or proto == "smb"):
            ...
            self.logger.info('CIFS LISTDIR path_elements: %s' %
                             str(path_elements))
        else:
        ...
        # for the source
        if(proto == "cifs" or proto == "smb"):
            fs = CifsFS('src', server=path_elements.hostname, share=path_elements.path,
                        username=path_elements.username, password=path_elements.password, is_dir=is_dir, recursive=recursive)
        elif (proto == "ftp"):
            fs = FtpFS('src', server=path_elements.hostname, path=path_elements.path,
                       username=path_elements.username, password=path_elements.password, is_dir=is_dir, recursive=recursive)
        elif (proto == 'file'):
            fs = FileFS('src', path=path_elements.path,
                        is_dir=is_dir, recursive=recursive)
        elif (proto == 'sftp'):
            fs = SshFS('src', server=path_elements.hostname, directory=path_elements.path,
                       username=path_elements.username, password=path_elements.password, is_dir=is_dir, recursive=recursive)
        else:
            q.eventhandler.raiseError(
                'Unsupported protocol [%s] for the path [%s]' % (proto, path))
