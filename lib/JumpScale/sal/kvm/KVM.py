...
class CloudMachine(Machine):
    ...
    def __init__(self, controller, name, os, disks, nics, memory, cpucount, uuid=None):
        self.pool = j.sal.kvm.Pool(controller, self.POOL)
        new_nics = list(map(lambda x: j.sal.kvm.Interface(controller, x,
            j.sal.kvm.Network(controller, x, x, [])), nics))
        new_disks = [j.sal.kvm.Disk(controller, self.pool, name, disks[0], os)]
        for i, disk in enumerate(disks[1:]):
            new_disks.append(j.sal.kvm.Disk(controller, self.pool, name, 'data-%s'%(i), disk))
