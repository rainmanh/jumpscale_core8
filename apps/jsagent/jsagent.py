...
class ProcessManager():
    def __init__(self,reset=False):
        ...
        ac_client_key = self.hrd.getList('producer.agentcontroller_client')[0]
        acclient = j.atyourservice.getServiceFromKey(ac_client_key)
        acclientinstancename = acclient.instance
        acconfig = acclient.hrd

        acip = acconfig.get("agentcontroller.client.addr",default="")

        if "hekad" in self.services:
            hekaServices = j.atyourservice.findservices(domain="jumpscale", role="hekad")
        ...
        if acip!="":
            ...
            nid=res["node"]["id"]
            jsagentService = j.atyourservice.getService(domain='jumpscale', role='jsagent', parent=None)
...
j.application.instanceconfig = j.atyourservice.getService(role='jsagent', instance=opts.instance).hrd
